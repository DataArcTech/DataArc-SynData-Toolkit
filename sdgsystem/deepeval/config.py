...
class PairwiseMetricConfig(BaseConfig):
    ...
    criteria: str = Field(
        default="Compare two model responses. Response A (actual_output) is from post-trained model. Response B (expected_output) is from base model. IMPORTANT: Neither response is ground truth - you must independently verify correctness.",
        description="Evaluation criteria"
    )
    evaluation_steps: List[str] = Field(
        default=[
