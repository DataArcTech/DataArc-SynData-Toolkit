"""
SDG Synthetic data generator for multi-modal distillation pipeline.
...
class SDGMMDistillation(BaseDistillation):
    """
    SDG Generator for creating multi-modal synthetic training data using LLMs.
    ...
    def generate(
        ...
        Args:
            ...
            image_paths: Optional list of image paths for multi-modal tasks
        ...
        batch_configs = []
        if image_paths:
            img_idx = 0  # local index

        for batch_idx in range(num_batches):
            remaining = num_samples - (batch_idx * batch_size)
            current_batch_size = min(batch_size, remaining)

            # pick one image for this batch
            if image_paths:
                image_path = image_paths[img_idx]
                img_idx = (img_idx + 1) % len(image_paths)  # loop
            else:
                image_path = None
    ...
    def _generate_single_batch(
        ...
        image_path = batch_config["image_path"]
        ...
        try:
            response = self.model.multimodal_generate(
                prompts=prompt,
