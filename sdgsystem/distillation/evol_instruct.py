...
class EvolInstructDistillation(BaseDistillation):
    """
    Evol-Instruct Generator for creating synthetic training data using LLMs.
    ...
    def generate(
        ...
        for batch_idx in range(num_batches):
            ...
            evol_direction = "in_breadth" if evol_direction == "in_depth" else "in_depth"
    ...
    def _build_batch_prompt(
        ...
        if evol_direction == "in_breadth":
            prompt = EVOL_INSTRUCT_BATCH_GENERATION_IN_BREADTH_PROMPT.format(
                task_instruction=self.task_instruction,
                demo_examples_section=demo_section,
                batch_size=batch_size
            )
        elif evol_direction == "in_depth":
            prompt = EVOL_INSTRUCT_BATCH_GENERATION_IN_DEPTH_PROMPT.format(
                task_instruction=self.task_instruction,
                demo_examples_section=demo_section,
                batch_size=batch_size
            )
